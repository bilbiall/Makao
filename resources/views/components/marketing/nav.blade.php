<header x-data="{ open: false }" class="sticky top-0 z-50 bg-stone-50/90 backdrop-blur border-b border-slate-200">
    <nav class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        <a href="{{ url('/') }}" class="text-xl font-bold text-slate-900">Renty</a>

        <div class="hidden md:flex items-center gap-8 text-sm font-medium text-slate-600">
            <a href="{{ url('/#features') }}" class="hover:text-slate-900">Features</a>
            <a href="{{ route('pricing') }}" class="hover:text-slate-900">Pricing</a>
            <a href="{{ route('generic.login') }}" class="hover:text-slate-900">Log in</a>
        </div>
        ...
        <button @click="open = !open" class="md:hidden p-2 text-slate-700" aria-label="Toggle menu">
