...
{{-- Semi-transparent + blurred (not flat bg-white) so this reads as sitting on the
     hero photo rather than clashing with it as an opaque block. Always exactly two
     rows - tabs, then every field inline on one row - shrunk down (no labels, icon-
     only submit) below `lg:`, full-size with labels and text from `lg:` up. --}}
<div x-data="{ tab: 'home' }" class="rounded-2xl border border-white/40 bg-white/75 backdrop-blur-md p-1.5 lg:p-2 shadow-lg shadow-black/10 w-full sm:mx-auto sm:max-w-md lg:mx-0 lg:max-w-none">
    ...
    <form x-show="tab === 'home'" x-cloak method="GET" action="{{ route('listings.index') }}" class="grid grid-cols-[1.3fr_1fr_0.9fr_auto] items-end gap-1.5 p-1.5 lg:gap-3 lg:p-3 lg:grid-cols-[1.2fr_1fr_1fr_auto]">
        <div class="min-w-0">
            ...
            <input id="h-area" name="area" list="areas-datalist" type="text" placeholder="Location" class="{{ $fieldClass }}" autocomplete="off">
        </div>
        <div class="min-w-0">
    ...
    <form x-show="tab === 'stay'" x-cloak method="GET" action="{{ route('stays.index') }}" class="grid grid-cols-[1.3fr_1fr_0.9fr_auto] items-end gap-1.5 p-1.5 lg:gap-3 lg:p-3 lg:grid-cols-[1.2fr_1fr_1fr_auto]">
        <div class="min-w-0">
            ...
            <input id="s-area" name="area" list="areas-datalist" type="text" placeholder="Location" class="{{ $fieldClass }}" autocomplete="off">
        </div>
        <div class="min-w-0">
    ...
    {{-- Type-ahead suggestions from real listed areas - not a hard restriction, so
         searching an area with no listings yet still runs and shows a clear empty state. --}}
