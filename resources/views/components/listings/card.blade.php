...
@endphp
<div class="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-shadow duration-200 hover:shadow-md">
            ...
            @if (auth()->user()->isUser())
                <form method="POST" action="{{ route('listings.watchlist', $house) }}" class="absolute right-3 top-3">
                    ...
                    <button type="submit" aria-label="{{ $watchlisted ? 'Remove from saved' : 'Save listing' }}" aria-pressed="{{ $watchlisted ? 'true' : 'false' }}"
                        class="grid h-9 w-9 place-items-center rounded-full bg-white/90 shadow-sm transition-colors {{ $watchlisted ? 'text-rose-600' : 'text-slate-600 hover:text-rose-600' }}">
                        @svg($watchlisted ? 'heroicon-s-heart' : 'heroicon-o-heart', 'w-4.5 h-4.5')
                    </button>
                </form>
            @endif
        @else
            <a href="{{ route('generic.login') }}" aria-label="Log in to save" class="absolute right-3 top-3 grid h-9 w-9 place-items-center rounded-full bg-white/90 text-slate-600 shadow-sm transition-colors hover:text-rose-600">
        ...
        <div class="flex items-start justify-between gap-3">
            <p class="text-lg font-semibold tracking-tight text-slate-900">
                @if ($isStay && $cheapestPackage)
                    KES {{ number_format($cheapestPackage->price) }}<span class="text-sm font-normal text-slate-500">/{{ $cheapestPackage->billing_unit }}</span>
                @else
                    KES {{ number_format($house->rent_amount) }}<span class="text-sm font-normal text-slate-500">/mo</span>
                @endif
            </p>
        </div>

        <h3 class="mt-1 truncate text-sm font-medium text-slate-900">{{ $house->house_name }}</h3>
