<x-layouts.marketing :title="'Find a house'">
    <div class="pb-14">
        <div class="sticky top-[57px] z-30 border-b border-slate-200 bg-stone-50/95 backdrop-blur">
            <div class="mx-auto max-w-6xl px-4 py-3 sm:px-6">
                <form method="GET" class="grid gap-2 sm:grid-cols-3 lg:grid-cols-[1fr_1fr_1fr_auto] lg:items-center">
                    @php $fieldClass = 'w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none transition-colors focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/20'; @endphp
                    <input type="text" name="area" list="areas-datalist" value="{{ request('area') }}" placeholder="Search a location..." autocomplete="off" class="{{ $fieldClass }}">
        ...
        <div class="mx-auto max-w-6xl px-4 py-6 sm:px-6">
            <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Homes to rent long-term</h1>
            ...
            @else
                <div class="mt-6 rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-sm">
                    ...
                    <h2 class="mt-4 text-lg font-semibold text-slate-900">No homes match these filters yet</h2>
                    ...
                    <div class="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
                        ...
                        <a href="{{ route('stays.index') }}" class="rounded-lg border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                            Browse short stays
                        </a>
                    </div>
                </div>
